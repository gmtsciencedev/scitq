    ...
    $.getJSON('/ui/get/', {
        ...
        for (i=0; i<workers.length; i++) {
            ...
            worker_table += '<tr class="" ><td><a type="button" class="btn btn-outline-dark border-0" target="_blank" href="/ui/task/?sortby=&worker='+workers[i].worker_id+'&batch="">'+workers[i].name
                    ...
                    +'</td><td>'+(workers[i].memory==null?'':workers[i].memory)
                    +(typeof(workers[i].stats)=='object'?('</td><td>'+(workers[i].stats.load)
                        ...
                        +'</td><td>'+(workers[i].stats.network.speed+'<br/>'+workers[i].stats.network.counter)
